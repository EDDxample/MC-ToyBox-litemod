...
import eddxample.toybox.RenderUtils;
...
public class VillageMarker
{
	...
	public static boolean population = false,
				   		  golem      = false,
				   		  lines      = false;
	public static int village_radius = 0, // 0 = OFF, 1 = DOTS, 2 = LINES, 3 = CIRCLE
			   		  door_radius    = 0;
	...
	public static void RenderVillages(float partialTicks)
	{
		...
		if(_radius && !radii.isEmpty())
		{
			for (Map.Entry<List<Integer>, Color> entry : radii.entrySet())
			{
				...
				if (population) RenderUtils.drawBox(d0, d1, d2, center.getX() - r, center.getY() - 4, center.getZ() - r, center.getX() + r, center.getY() + 4, center.getZ() + r, color);
	            
				/* === SPHERES === */
				
	            if (village_radius != 0) RenderUtils.drawSphere(r, sphereDensity*2, d0, d1, d2, center.getX(), center.getY(), center.getZ(), color, village_radius);
	            if (door_radius != 0) RenderUtils.drawSphere(r + 32, sphereDensity*2, d0, d1, d2, center.getX(), center.getY(), center.getZ(), color, door_radius);
			}
		}
	...
	{
		centers.clear(); radii.clear(); doors.clear();
